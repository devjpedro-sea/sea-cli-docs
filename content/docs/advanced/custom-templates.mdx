...
Aprenda a criar templates customizados para adaptar o SEA CLI às necessidades específicas do seu projeto ou organização.

## Visão Geral

O SEA CLI usa templates para gerar arquivos de configuração. Você pode criar seus próprios templates para:

- 📝 Personalizar regras ESLint/Prettier
- 🎨 Adaptar configurações de teste
- 🏢 Aplicar padrões da empresa
...
1. Templates locais (.sea/templates/)
   ↓ sobrescreve
2. Templates do usuário (~/.sea/templates/)
   ↓ sobrescreve
3. Templates padrão (SEA CLI)
```
...
module.exports = {
  ...
  overrides: [
    // Custom: Regras específicas para testes
    {
...
module.exports = {
  ...
  collectCoverageFrom: [
    ...
    "!src/index.ts",
    "!src/**/*.stories.tsx",
    // Custom: Ignorar pastas específicas
    "!src/generated/**",
    "!src/types/**",
  ],
  coverageThreshold: {
...
# Custom: Verificar imports não utilizados
...
Use variáveis para criar templates dinâmicos:
...
description: Template padrão da Empresa XYZ
version: 1.0.0
...
module.exports = {
  validate(template) {
    ...
    if (!template.version) {
      errors.push("Template deve ter uma versão");
    }
...
describe("Template ESLint", () => {
  it("deve gerar .eslintrc.js válido", () => {
  ...
  it("deve aplicar variáveis corretamente", () => {
...
module.exports = {
  ...
  rules: {
    // Regras específicas para microserviços
...
<Callout type="tip">
  ### Recomendações - **Versionamento**: Sempre versione seus templates -
  **Documentação**: Documente variáveis e opções disponíveis - **Validação**:
  Valide templates antes de distribuir - **Testes**: Teste templates em
  diferentes cenários - **Manutenção**: Mantenha templates atualizados com as
  versões das ferramentas - **Compartilhamento**: Use repositórios Git para
  compartilhar templates - **Modularidade**: Crie templates reutilizáveis e
  componíveis
</Callout>

## Troubleshooting

### Template Não Aplicado
...
# Fornecer variáveis explicitamente
sea init --template custom \
  --var PROJECT_NAME=meu-projeto \
  --var AUTHOR="João Silva"
```
...
**Customize o SEA CLI para suas necessidades!** Templates flexíveis e reutilizáveis.
