...
Configura Turborepo no workspace Liferay para otimizar builds, testes e deploys em monorepos com múltiplos módulos e client extensions.
...
O comando `create-turbo` automatiza a configuração do Turborepo:
...
7. Adiciona configurações CI/CD

**Benefícios:**
- ⚡ Builds até 10x mais rápidos
- Cache inteligente de artifacts
- Execução paralela de tasks
- 🎯 Build apenas do que mudou
- ☁️ Cache remoto compartilhado
...
Encontrados 12 módulos em modules/
Encontrados 5 client extensions em client-extensions/
...
.turbo/
└── cache/
    ├── modules-produto-portlet-build-abc123.tar.gz
    ├── modules-usuario-api-test-def456.tar.gz
    └── ...
```
...
Turborepo detecta e constrói na ordem correta.
...
// package.json (root)
{
  "workspaces": [
    "modules/*",
    "client-extensions/*",
    "packages/*"  // Adicione padrões faltando
  ]
}
```
...
Total: 2s (39x mais rápido)
```

### Build Incremental

```
Mudança em produto-portlet:
- Rebuild: 1 workspace
- Cache: 4 workspaces

Total: 6s (13x mais rápido)
```
...
```bash
# Build apenas módulos de produto
...
**Builds 10x mais rápidos com Turborepo!** Cache inteligente, execução paralela, build incremental.
