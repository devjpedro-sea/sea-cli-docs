---
title: API Reference
description: Referência completa da API do SEA CLI
---

# API Reference

Documentação técnica da API do SEA CLI para uso programático e integração.
...
interface SeaCLIOptions {
  ...
  dryRun?: boolean; // Simulação (não executa)
}
...
interface CreateWorkspaceOptions {
  ...
  liferayVersion?: string; // Versão do Liferay
  javaVersion?: string; // Versão do Java
...
interface SetupOptions {
  ...
  skipInstall?: boolean; // Pular instalação de dependências
}
...
if (result.valid) {
  ...
  console.error("Invalid project name:", result.errors);
}
```
...
async function setupProject() {
  ...
  try {
    ...
    Logger.info("⚛️  Creating client extension...");
    await cli.createClientExtension({
      name: "catalogo-ce",
      type: "customElement",
      framework: "react",
    ...
    Logger.info("Setting up quality tools...");
    await cli.setup({
  ...
  if (error instanceof SeaError) {
    switch (error.code) {
      case ErrorCodes.INVALID_CONFIG:
        console.error("Configuração inválida:", error.details);
        break;
      case ErrorCodes.FILE_NOT_FOUND:
        console.error("Arquivo não encontrado:", error.details);
        break;
      default:
        console.error("Erro:", error.message);
    }
...
<Callout type="tip">
  ### Recomendações - **Type Safety**: Use TypeScript para melhor DX - **Error
  Handling**: Sempre trate erros adequadamente - **Logging**: Use Logger ao
  invés de console.log - **Async/Await**: Prefira async/await sobre Promises -
  **Validação**: Valide inputs antes de processar - **Cleanup**: Limpe recursos
  em caso de erro
</Callout>
...
**API programática para automação completa!** 
